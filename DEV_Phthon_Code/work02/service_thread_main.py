...
def setCAPImagelist(frame):
    ...
    try:
        ...
        nowfile = './resource/CAP_IMG_THREAD' + str(imageBufferIdx) + ".jpg"
        rzTmp = cv2.resize(globalMainImage[imageBufferIdx], (100, 75))
        cv2.imwrite(nowfile, rzTmp)

    finally:

        print("이미지 배열 IDX ", imageBufferIdx, len(globalMainImage))
...
def saveimage(frame):
    dt = datetime.now()
    nowfile =  './resource/CAP_IMG_THREAD.jpg'
...
def getMatchTemplate(keyIdx):
    ...
    try:
        ...
        time.sleep(loopSleepTime)
        print("Threading [",i,"] matchTemplate Exception ")
...
if __name__ == '__main__':
    ...
    #쓰레드를 arrayRang 배열 갯수만큼 생성 함
    # for i in range(len(globalMainImage)):
    #     my_thread = threading.Thread(
    #         target=getMatchTemplate, args=(i,))
    #     my_thread.start()
    ...
    if cap.isOpened() :
        ...
        while (cap.isOpened()):
            ...
            if (ret):
                ...
                # my_thread= threading.Thread(
                #     target=saveimage, args=(gray.copy(),))
                # my_thread.start()
